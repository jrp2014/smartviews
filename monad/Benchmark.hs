...
import qualified Control.Monad.Free.Reflectable
                                               as FR
import qualified Control.Monad.Operational     as O
import qualified Control.Monad.Operational.Reflectable
                                               as OR
...
interleave v v' = case view v of
  Var a   -> fmap (a, ) v'
  Con v'' -> wrap (fmap (fmap sw . interleave v') v'')
    where sw (x, y) = (y, x)
...
test2S n = feedAll
  (interleave (addNbad n) (addNbad n) :: SF.Free Get (Int, Int))
  [1 .. n]
...
test2Ref n = feedAll
  (interleave (addNbad n) (addNbad n) :: FR.FreeMonad Get (Int, Int))
  [1 .. n]

test2Oper :: Int -> Maybe (Int, Int)
test2Oper n = feedAll
  (interleave (addNbad n) (addNbad n) :: O.Program Get (Int, Int))
  [1 .. n]

test2 =
  ( "monad-interleave"
  , [("SplayFree", test2S), ("Ref", test2Ref), ("Oper", test2Oper)]
  )

benchgroupNs (name, is) ns =
  [ bgroup (name ++ "/" ++ show n) [ bench imp (nf fun n) | (imp, fun) <- is ]
  | n <- ns
  ]
