{-# LANGUAGE ExistentialQuantification, DeriveFunctor, ViewPatterns, FlexibleInstances #-}
...
data FMP f x  = FZero
              | FCons (FFMP f x) (FMP f x)
              | (FMP f x) :+ (FMP f x)
              | forall a . (FMP f a) :>>= (a -> FMP f x)

data FFMP f x = FVar x | FCon (f (FMP f x))
...
lift x = con (x >>= return . return)


data MonadPlusView f x = MZero | MCons (FFMP f x) (FMP f x)
...
monadPlusView (x :+ y)             = case monadPlusView x of
...
monadPlusView (m :>>= f)           = case monadPlusView m of
                                       MZero              -> MZero
                                       MCons (FVar x) xs  -> monadPlusView (f x :+ (xs :>>= f))
                                       MCons (FCon t) xs  -> MCons (FCon (t >>= return . (:>>= f))) (xs :>>= f)
...
instance Monad m => MonadLogic (FMP m) where
  msplit x = case mpv x of
            MZero              -> return Nothing
            MCons (FVar a) xs  -> return (Just (a,xs))
            MCons (FCon m) xs  -> FCons (FCon (m >>= return . msplit)) (msplit xs)

observeAllT :: (Monad m) => FMP m a -> m [a]
observeAllT (mpv -> MZero) = return []
observeAllT (mpv -> MCons (FVar v) xs) = liftM (v:) (observeAllT xs)
